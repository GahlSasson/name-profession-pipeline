...
def pick_one(avail, aliases, allow_contains=True):
...
def fetch_all(base_url, H_AUTH, page_size=100, max_pages=1000):
    ...
    while True:
        ...
        data = r.json(); recs.extend(data.get("records", []))
        offset = data.get("offset"); pages += 1
...
def learn_pmi(records, nm_f, gn_f, sn_f, label_f):
    token_counts, label_counts = Counter(), Counter()
    ...
    for rec in records:
        ...
        N += 1; label_counts[label] += 1
        for t in toks:
            token_counts[t] += 1; joint_counts[label][t] += 1
    if N == 0: return {}, [], 0
    alpha = 1.0
    labels = list(label_counts.keys()); tokens = list(token_counts.keys())
...
def score(tokens, pmi, labels):
    ...
    for t in tokens:
        for c in labels:
            ...
            if v:
                scores[c] += v; contrib[c].append((t, v))
    ranked = sorted(scores.items(), key=lambda kv: kv[1], reverse=True)
    best, best_s = ranked[0]; second_s = ranked[1][1] if len(ranked)>1 else 0.0
    gap = best_s - second_s
    top = sorted(contrib[best], key=lambda kv: kv[1], reverse=True)[:5]
    return best, gap, top

def truthy(val):
    if isinstance(val, bool): return val
    if isinstance(val, (int, float)): return val != 0
    return str(val).strip().lower() in {"1","true","yes","y","checked"}

# ---------- main ----------
def main():
    THRESHOLD = float(os.getenv("THRESHOLD", "0.8"))
    DRY_RUN   = os.getenv("DRY_RUN","false").strip().lower() in {"1","true","yes"}
    FORCE_LABEL = os.getenv("FORCE_LABEL","").strip().lower()   # "", "profession", or "cluster"
    ...
    # label candidates
    cl_exact = pick_one(avail, ["profession_cluster","professional_cluster","Profession Cluster"], allow_contains=False)
    pr_exact = pick_one(avail, ["profession_canonical","professional_canonical","professional_canonoical","occupation","Occupation","Job","Role"], allow_contains=False)
    # allow contains as fallback, but ignore fields that look disabled
    cl_cont  = pick_one(avail, ["cluster","Cluster"], allow_contains=True)
    if cl_cont and ("disable" in cl_cont.lower()): cl_cont = None
    cl_f = cl_exact or cl_cont
    pr_f = pr_exact

    # FORCE_LABEL overrides
    if FORCE_LABEL == "profession": cl_f = None
    if FORCE_LABEL == "cluster":    pr_f = None

    # choose label
    label_f   = cl_f if cl_f else pr_f
    label_kind= "cluster" if cl_f else "profession" if pr_f else None
    if not label_f:
        sys.exit("[analyzer] No label field found (cluster or profession_canonical).")
    ...
    print(f"[cfg] THRESHOLD={THRESHOLD} DRY_RUN={DRY_RUN} FORCE_LABEL={FORCE_LABEL or '(auto)'}")
    ...
    for rec in rows:
        f = rec.get("fields", {}); rec_id = rec.get("id")
        if lock_f and lock_f in f and truthy(f.get(lock_f)): locked += 1; continue
        toks = name_tokens(f.get(nm_f, "") if nm_f else "", f.get(gn_f, "") if gn_f else "", f.get(sn_f, "") if sn_f else "")
        if not toks: continue
