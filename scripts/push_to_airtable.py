...
# --------- Helpers to align script fields to your actual Airtable schema ---------

def norm(name: str) -> str:
    """normalize a field name for fuzzy matching (lowercase, hyphen/space -> underscore)."""
    return re.sub(r'[^a-z0-9]+', '_', (name or '').strip().lower())

def fetch_table_fields(base_id: str, table_id_or_name: str, key: str):
    """Return (table_id, table_name, [field_names]) for the given table id/name from meta API."""
    url = f"{API}/meta/bases/{base_id}/tables"
    h = {"Authorization": f"Bearer {key}"}
    r = requests.get(url, headers=h)
    if r.status_code >= 300:
        print("Meta fetch error:", r.status_code, r.text)
        raise SystemExit(1)
    data = r.json()
    ...
    print("Could not find table in meta:", table_id_or_name)
    raise SystemExit(1)

def build_field_mapping(available_fields):
    """
    Build a mapping from our canonical keys to the actual Airtable field names.
    Mapping is case-insensitive and treats '-' and ' ' like '_'.
    """
    avail_by_norm = {norm(f): f for f in available_fields}
    # Canonical keys we might send
    canonical = [
    ...
    mapping = {}
    # default hints for common mismatches in your base
    hints = {
        "status": "Status",           # your base uses capital S
        "match_type": "match-type"    # your base uses hyphen
    }
    for k in canonical:
        k_norm = norm(k)
        # 1) try exact normalized match
        if k_norm in avail_by_norm:
            mapping[k] = avail_by_norm[k_norm]
            continue
        # 2) try hint
        if k in hints and norm(hints[k]) in avail_by_norm:
            mapping[k] = avail_by_norm[norm(hints[k])]
            continue
        # 3) else leave unmapped (we'll skip it when sending)
    print("Field mapping (canonical -> table):")
    for ck, tk in mapping.items():
        print(f"  {ck} -> {tk}")
    # also show what we will skip (for visibility)
...
def map_row_to_fields(r, mapping):
    """Convert a CSV row into Airtable fields using the mapping; skip unknowns."""
    # compute match_type value
    try:
    ...
    # apply mapping; only include fields that exist in the table
    out = {}
    for ck, val in canonical_fields.items():
        tk = mapping.get(ck)
        if tk:
            out[tk] = val
    return out

# --------- Read existing ids (to upsert) ---------
...
if __name__=="__main__":
    ...
    # find actual table fields and build mapping
    table_id, table_name, field_names = fetch_table_fields(base, table, key)
    mapping = build_field_mapping(field_names)
    if "id" not in mapping:
        print("❌ Your table must have an 'id' field to track the external person URL. Please add a text field named 'id'.")
        raise SystemExit(1)
